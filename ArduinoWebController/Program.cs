...
app.MapGet("/", () =>
{
    ...
    html += "                <button id=\"powerButton\" class=\"power-button\" onclick=\"sendCommand('on')\" style=\"flex: 0 0 120px;\">POWER ON</button>\n";
    html += "                <div id=\"gameInfo\" style=\"font-size: 14px; text-align: center;\">\n";
    html += "                    <div>Attempts: <span id=\"attempts\">3</span></div>\n";
    html += "                    <div>Score: <span id=\"score\">0</span></div>\n";
    html += "                </div>\n";
    html += "                <div id=\"status\" class=\"status\" style=\"flex: 1; margin-left: 10px;\"></div>\n";
    html += "            </div>\n";
    html += "            <div style=\"display: flex; justify-content: center; margin-top: 20px;\">\n";
    html += "                <button class=\"lock-button\" onclick=\"sendCommand('lock')\" style=\"width: 60%; font-size: 20px;\">LOCK-IN</button>\n";
    html += "            </div>\n";
    html += "        </div>\n";
    html += "\n";
    html += "        <div class=\"panel\">\n";
    ...
    html += "\n";
    html += "    <script>\n";
    html += "        // Track game state locally\n";
    html += "        let gameState = {\n";
    html += "            attemptsLeft: 3,\n";
    html += "            score: 0,\n";
    html += "            isPoweredOn: false\n";
    html += "        };\n";
    html += "\n";
    html += "        // Initialize UI\n";
    html += "        document.addEventListener('DOMContentLoaded', function() {\n";
    html += "            updateUIFromState();\n";
    html += "        });\n";
    html += "\n";
    html += "        function updateUIFromState() {\n";
    html += "            document.getElementById('attempts').textContent = gameState.attemptsLeft;\n";
    html += "            document.getElementById('score').textContent = gameState.score;\n";
    html += "        }\n";
    html += "\n";
    ...
    html += "                    \n";
    html += "                    // Update local game state based on command\n";
    html += "                    if(command === 'on') {\n";
    html += "                        gameState.isPoweredOn = true;\n";
    html += "                        gameState.attemptsLeft = 3;\n";
    html += "                        gameState.score = 0;\n";
    ...
    html += "                        gameState.isPoweredOn = false;\n";
    ...
    html += "                    } else if(command === 'lock' && gameState.isPoweredOn && gameState.attemptsLeft > 0) {\n";
    html += "                        gameState.attemptsLeft--;\n";
    html += "                    }\n";
    html += "                    \n";
    html += "                    updateUIFromState();\n";
    html += "                    refreshDebug();\n";
    ...
    html += "\n";
    html += "                    // Parse debug data to update game info\n";
    html += "                    updateGameInfo(data);\n";
    html += "                });\n";
    html += "        }\n";
    html += "\n";
    html += "        function updateGameInfo(debugData) {\n";
    html += "            // Only look for score in the debug output, attempts are tracked client-side\n";
    html += "            \n";
    html += "            // Process line by line to find the latest score value\n";
    html += "            const lines = debugData.split('\\n');\n";
    html += "            for (let i = lines.length - 1; i >= 0; i--) {\n";
    html += "                const line = lines[i];\n";
    html += "                \n";
    html += "                // Check for SCORE pattern (both regular and final score)\n";
    html += "                if (line.includes('SCORE:') || line.includes('FINAL_SCORE:')) {\n";
    html += "                    const match = line.match(/(SCORE|FINAL_SCORE):\\s*(\\d+)/);\n";
    html += "                    if (match && match[2]) {\n";
    html += "                        // Update game state\n";
    html += "                        gameState.score = parseInt(match[2]);\n";
    html += "                        updateUIFromState();\n";
    html += "                        console.log('Found score:', match[2]);\n";
    html += "                        break; // Exit after finding the most recent score\n";
    html += "                    }\n";
    html += "                }\n";
    html += "                \n";
    html += "                // Check for game reset or completed messages\n";
    html += "                if (line.includes('GAME RESET') || line.includes('GAME STARTED')) {\n";
    html += "                    gameState.attemptsLeft = 3;\n";
    html += "                    gameState.score = 0;\n";
    html += "                    updateUIFromState();\n";
    html += "                    console.log('Game reset detected');\n";
    html += "                    break;\n";
    html += "                }\n";
    html += "            }\n";
    html += "        }\n";
